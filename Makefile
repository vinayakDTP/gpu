...
BUSYBOX_TARBALL := download/busybox-$(BUSYBOX_VERSION).tar.bz2
...
mesa: $(MESA_SRC)
	mkdir -p build/mesa
	PATH="$(CURDIR)/build/buildroot/host/bin/:$(CURDIR)/buildroot/host/sbin:$(PATH)" \
	PYTHONNOUSERSITE=y meson setup \
		--prefix=/usr \
		--libdir=lib \
		--default-library=shared \
		--buildtype=debug \
		--cross-file=$(CURDIR)/cross-compilation.conf \
		-Db_pie=false \
		-Db_staticpic=true \
		-Dstrip=false \
		-Dbuild.pkg_config_path=$(CURDIR)/build/buildroot/host/lib/pkgconfig\
		-Dbuild.cmake_prefix_path=$(CURDIR)/build/buildroot/host/lib/cmake \
		...
		-Dzstd=disabled \
		-Dglvnd=false \
		$(CURDIR)/src/mesa-24.0.9/ $(CURDIR)/build/mesa/
...
busybox: $(BUSYBOX_SRC)
	...
	cp -v busybox_config build/busybox/.config
	$(MAKE) -C $(BUSYBOX_SRC) O=$(CURDIR)/build/busybox
	$(MAKE) -C $(BUSYBOX_SRC) O=$(CURDIR)/build/busybox install CONFIG_PREFIX=$(CURDIR)/build/ramdisk
